...
function [pxs,pys,pzs]=min_jerk_ineq(ts,X0,Xdot0,Xddot0,waypoint_polygon_seq,corridor_polygon_seq,w_pnts)
...
% pxs = (poly_order + 1) x n_seg
    ...
    for n=1:n_seg
     ...
     %% soft waypoint cost 
     % in case of waypoint cost, we penalize the deviation from the center
     % of polygon 
        
    xs = X0(1); ys  = X0(2); zs = X0(3);    
...
%    disp(score) 
   ...
   insert_range1=1:poly_order+1; insert_range2=insert_range1+poly_order+1;
    ...
    %% QP formulation 
    % objective function
    Q = blkdiag(Q_jerk,Q_jerk,Q_jerk) + w_pnts * blkdiag(Q_wpnt,Q_wpnt,Q_wpnt); % jerk matrix
