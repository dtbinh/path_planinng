...
% color_scale = flipud(jet(ceil(max(vis_cost_set(draw_t_step,:))-min(vis_cost_set(draw_t_step,:)))));
color_scale = (jet(200));
color_debug = [];
for azim_idx = 1:N_azim
    azim = angles(azim_idx);
    cost = vis_cost_set(draw_t_step,azim_idx);
    th1 = azim - dtheta; th2 = azim+dtheta;
    if(DT_set(draw_t_step,azim_idx)>1)
        coll = ceil((max_cost-1 - cost)/(max_cost-1-min_cost)*200);
        draw_circle_sector(xc,th1,th2,ray_len,color_scale(coll,:),0.2,'none');
    end
end
